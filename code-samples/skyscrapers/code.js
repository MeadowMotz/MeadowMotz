...
// puzzle generator helper
// determine visible buildings from edge
function label(table, size) {
  ...
  for (let row = 0; row < size; row++) {
      const realRow = table.rows[row + 1]; // shift for top labels
...
// puzzler generator helper; fills with random (validated) numbers
...
// Clear numbers from table cells after generation
function hideNumbers(cells, numberArray) {
  for (let i = 0; i < cells.length; i++) {
    for (let j = 0; j < cells[i].length; j++) {
      cells[i][j].textContent = ""; // clear
    }
  }
  console.log("Secret number array:", numberArray);
}

// test user puzzle solution
function validate() {
    const size = parseInt(document.getElementById("shape").value);
...
// helper to determine number of correct and incorrect inputs
...
// count visible buildings by puzzle rules
